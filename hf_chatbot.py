...
class HuggingFaceChatBot:
    def __init__(self):
        ...
        self.cookie_path_dir = (
            "./ENV/cookies/"  # Ensure trailing slash (/) for directory path
        )
        self.cookies = None
        self.chatbot = None
    ...
    def login_to_huggingface(self):
        ...
        try:
            ...
            self.cookies = sign.login(
                cookie_dir_path=self.cookie_path_dir, save_cookies=True
            )
            return True
    ...
    def ensure_chatbot_initialized(self):
        ...
        if not self.chatbot:
            ...
            self.chatbot = (
                hugchat.ChatBot(cookies=self.cookies.get_dict())
                if self.cookies
                else None
            )
        ...
        try:
            ...
            convo = (
                self.chatbot.new_conversation(
                    system_prompt="Translate in Persian and summarize the provided text into exactly 32 words, maintaining a friendly news tone. Ensure the translation accurately reflects the original content without any first-person perspective, using only plural pronouns. At The End, include two related emojis side by side. Display only the translated and summarized text along with the emojis; avoid adding any extra content."
                )
                if self.chatbot
                else Conversation()
            )
            message_result = (
                self.chatbot.chat(prompt, conversation=convo) if self.chatbot else None
            )
